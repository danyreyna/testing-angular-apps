...
- But it's still not capable of simulating things like:
  ...
  - geolocation
  - media queries
- For this, you have to mock `window.resizeTo`, `window.navigator.geolocation.getCurrentPosition`, and polyfill `window.matchMedia`.
...
<div style={{fontSize: '0.8em'}}>
- All functions exported from `math.ts` are converted to mock functions.
- We can call `.mockImplementation(() => {})` on them.
...
<div style={{ fontSize: '0.8em' }}>
- Call `vi.importActual('some-path')`, spread the imported members, and override what you need to.
...
- mock browser APIs
- mock modules
- how to add a custom render method that includes common providers, like http stuff
- how to test services
- additional test tips notes
